...
| B：Qwen2.5-1.5B | HuggingFace 预训练模型 | QLoRA → 导出 + 对话 | 验证工程实践 |
...
| 数据集 | 用途 | 规模 |
|--------|------|------|
| minimind_dataset | 预训练 + SFT + DPO | 1.41M 条 |
| generated_chat_0.4M | 备用 SFT 数据 | 400K 对话 |
...
### 轨道 B：HuggingFace 导出 + CLI 对话（Qwen2.5-1.5B）

```bash
# 命令行对话
python inference/chat.py --checkpoint outputs/sft_qlora/ckpt_final.pt
```
...
│   ├── serve_vllm.py          # 轨道 B：vLLM 服务部署（需 Linux/WSL2）
│   ├── smoke_vllm.py          # 轨道 B：vLLM 验证（需 Linux/WSL2）
...
├── docs/               # 文档（架构/DPO理论/训练笔记/项目总结）
├── results/            # 实验结果 + 生成对比
├── data/               # 数据集（gitignore）
│   ├── minimind_dataset/      # 轨道 A 数据
│   ├── generated_chat_0.4M/   # 轨道 A 备用 SFT 数据
